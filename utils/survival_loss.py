...
# ----- NLL discrete survival helpers（与 MIL_clinical/utils.py 中分桶逻辑一致）-----


def compute_nll_bin_edges(train_times: np.ndarray, n_bins: int) -> np.ndarray:
    """用训练集生存时间的分位数构造 bin 边界，长度 n_bins+1。"""
...
def map_time_to_bins_with_edges(time_tensor: torch.Tensor, edges: np.ndarray) -> torch.Tensor:
    """连续生存时间 -> 离散 bin 下标，与 MIL_clinical/utils.map_time_to_bins_with_edges 一致。"""
...
def discrete_survival_logits_to_risk(logits: torch.Tensor) -> torch.Tensor:
    """
    NLL 离散生存头 logits (B, K) -> 用于 C-index 的 risk（与 MIL_clinical get_survival_eval_metrics 一致）。
    hazards = sigmoid(logits)，S 为累积生存，risk = -sum(S)。
    """
    if logits.dim() == 1:
